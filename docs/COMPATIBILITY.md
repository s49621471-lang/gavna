...
| Sharing a file *into* a guest | `NOT_TESTED` | `NOT_TESTED` | A host app returning a `content://` URI should work — the grant is to UNIQUE's uid, which is the uid the guest runs as — but it has not been exercised |
