...
class VirtualLaunchTest {
    ...
    fun t21_theGuestReadsItsOwnSignatureAndTheTruthAboutGms() = runBlocking {
        ...
        // Play services is the one exception, and it is deliberate: a guest is always
        // told it is absent, whether or not the device has it. GMS refuses a virtual
        // app's identity and kills it from a `Handler` — `SecurityException: Unknown
        // calling package name` — so "absent" is both what the SDKs are built to handle
        // and the truth about what the guest can use. See `VirtualPackageManagerHook`.
        assertThat(observed["present.com.google.android.gms"]).isEqualTo("false")
        // Play *Store* is not hidden: the licence check is an ordinary bind that works,
        // and a PAIRIP-protected app exits when it cannot reach it.
