...
| | |
|---|---|
| **Off-device tests** | 290 JVM tests, 92 native checks, 15 Dart tests, 109 tool tests — all passing |
| **On-device suite** | **46 of 46** instrumented tests pass, on an Android 14 x86_64 emulator |
| **On a real phone** | Thirteen runs. **The eleventh ran a game to its own login screen and took a touch on it** — Standoff 2, Unity 6 on IL2CPP, its expansion file read out of the instance's own storage, FMOD, Firebase and AppMetrica all up, drawing at 2400×1080 on the hardware renderer. Sixty-odd causes found and fixed across the thirteen runs and the emulator work between them; the thirteenth passes seventeen of eighteen analyzer checks, and what the last one fails on is named to the byte. The changes made since it are not yet back on hardware |
...
./tools/native-test/run.sh        # 92 host-side native checks, no device needed
(cd ui && flutter test)           # 15 Dart tests
./tools/device-log/self_test.py   # 92 tests for the device-log analyzer, no toolchain
