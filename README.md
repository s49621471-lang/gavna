...
| | |
|---|---|
| **Off-device tests** | 269 JVM tests, 65 native checks, 15 Dart tests, 99 tool tests — all passing |
| **On-device suite** | **46 of 46** instrumented tests pass, on an Android 14 x86_64 emulator |
| **On a real phone** | Eleven runs. **The eleventh ran a game to its own login screen and took a touch on it** — Standoff 2, Unity 6 on IL2CPP, its expansion file read out of the instance's own storage, FMOD, Firebase and AppMetrica all up, drawing at 2400×1080 on the hardware renderer. Sixty-odd causes found and fixed across the eleven runs and the emulator work between them; the last several are not yet back on hardware |
...
with different evidence, and is the one this pass began on. The engine can now be read out
of a guest's own `/proc/self/maps` in two lines, for the cost of one `fopen` and no
permission; that file is answered from a rewritten view, the graft checks its own work and
reports what still leaks, and what the view does **not** cover is written down rather than
left to be found.
...
| **An app can still tell it is virtualized, by several routes** | `/proc/self/maps` inside a `:vappN` names UNIQUE's APK and UNIQUE's private directory, which is two lines and one `fopen`. That file is now answered from a rewritten view for the guest's own native libraries — where anti-cheat code lives — and the graft checks its own work and reports what still leaks (`PROC_VIEW_INSTALLED … leaked=0`). Four routes it does **not** cover, stated rather than discovered: a read made through Java (`libjavacore.so` is a system library UNIQUE does not patch), `dl_iterate_phdr`, a raw `syscall(SYS_openat, …)`, and `ApplicationInfo.dataDir`, which is the virtual path and has to be until Java file IO is redirected too. `docs/STATUS.md` has the design and the next step. |
...
./tools/native-test/run.sh        # 65 host-side native checks, no device needed
(cd ui && flutter test)           # 15 Dart tests
./tools/device-log/self_test.py   # 82 tests for the device-log analyzer, no toolchain
